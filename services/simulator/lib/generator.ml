...
let generate_next_price current_price model ~dt =
  ...
  | RandomWalk {volatility; drift} ->
      let random_shock =
        generate_normal_random () *. volatility *. Float.sqrt dt
      in
  ...
  | MeanReverting {mean; reversion_speed; volatility} ->
      ...
      in
      let random_shock =
        volatility *. Float.sqrt dt *. generate_normal_random ()
      in
      current_price +. mean_reversion +. random_shock
  | Jump {normal_vol; jump_prob; jump_size} ->
      let normal_return =
        normal_vol *. Float.sqrt dt *. generate_normal_random ()
      in
      let jump =
        if Float.(Random.float 1.0 < jump_prob) then
...
let generate_quote instrument ~current_price ~timestamp =
  let spread_bps = 5.0 +. Random.float 10.0 in
  (* 5-15 bps spread *)
  let bid, ask = generate_bid_ask ~mid_price:current_price ~spread_bps in
  let bid_size =
    Float.of_int (100 + Random.int 900) *. instrument.lot_size
  in
  let ask_size =
    Float.of_int (100 + Random.int 900) *. instrument.lot_size
  in
...
let generate_trade instrument ~current_price ~timestamp =
  let trade_price =
    current_price *. (1.0 +. (Random.float 0.002 -. 0.001))
  in
  let quantity =
    Float.of_int (100 + Random.int 400) *. instrument.lot_size
  in
