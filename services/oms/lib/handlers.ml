...
type new_order_request = {order: Order.order; account_id: string}
[@@deriving yojson]

type order_response =
  {order_id: string; status: [`Accepted | `Rejected]; reason: string option}
[@@deriving yojson]

type fill_notification =
  {order_id: string; fill_qty: float; fill_price: float}
[@@deriving yojson]

type cancel_request = {order_id: string} [@@deriving yojson]
...
let handle_new_order state nats_client request =
  ...
  | Ok validated_order ->
      ...
      let* () =
        ...
        in
        Nats_client.publish_string nats_client ~subject:"orders.accepted"
          event_json
      in
      (* Send to risk check *)
      let* () =
        let risk_request =
          Yojson.Safe.to_string
            (`Assoc
               ...
               ; ( "side"
                 , `String
                     ( match validated_order.side with
                     | Buy -> "buy"
                     | Sell -> "sell" ) ) ] )
        in
        Nats_client.publish_string nats_client ~subject:"risk.check_request"
          risk_request
      in
      Lwt.return
        (Ok {order_id= validated_order.id; status= `Accepted; reason= None})
  | Error err ->
      let reason =
        match err with
        | `MaxQuantityExceeded (qty, max) ->
            Printf.sprintf "Quantity %.2f exceeds max %.2f" qty max
        | `InvalidSymbol sym -> Printf.sprintf "Invalid symbol: %s" sym
        | `InsufficientBuyingPower (req, avail) ->
            Printf.sprintf "Insufficient buying power: need %.2f, have %.2f"
              req avail
        ...
        let event_json =
          Yojson.Safe.to_string
            (`Assoc
               ...
               ; ("timestamp", `Float (Unix.gettimeofday ())) ] )
        in
        Nats_client.publish_string nats_client ~subject:"orders.rejected"
          event_json
      in
      Lwt.return
        (Ok
...
let handle_fill_notification state nats_client notification_json =
  ...
  | Some order -> (
    match
      Transitions.transition_to_filled order ~fill_qty ~fill_price
            ...
            (function
              | Transitions.PublishEvent event ->
                  ...
                  Nats_client.publish_string nats_client
                    ~subject:"orders.filled" event_json
              | Transitions.UpdatePosition pos ->
                  ...
                  in
                  Nats_client.publish_string nats_client
                    ~subject:"positions.update" position_json
              | Transitions.UpdateBalance _amount -> Lwt.return_unit )
            side_effects
        in
        Lwt.return (Ok ())
    | Error err -> Lwt.return (Error (Transitions.show_transition_error err))
    )
  | None -> Lwt.return (Error "Order not found")

let handle_cancel_request state nats_client request_json =
  let request =
    match
      cancel_request_of_yojson (Yojson.Safe.from_string request_json)
    with
    | Ok req -> req
  ...
  | Some order -> (
    match
      Transitions.transition_to_cancelled order
        ~timestamp:(Unix.gettimeofday ())
    with
    | Ok (updated_order, side_effects) ->
        let* () = State.update_order state updated_order in
          ...
          Lwt_list.iter_p
            (function
              | Transitions.PublishEvent (OrderCancelled cancel) ->
                  ...
                  in
                  Nats_client.publish_string nats_client
                    ~subject:"orders.cancelled" event_json
              | _ -> Lwt.return_unit )
            side_effects
        in
        Lwt.return (Ok ())
