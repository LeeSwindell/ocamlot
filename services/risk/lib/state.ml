...
let update_position state account_id ~instrument_id ~quantity ~side ~price =
  ...
  let new_pos =
    ...
    | Some pos, Types.Sell ->
        let new_qty = pos.quantity -. quantity in
        Checks.{pos with quantity= new_qty; market_value= new_qty *. price}
  in
