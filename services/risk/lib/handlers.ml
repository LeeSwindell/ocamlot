...
let handle_risk_check_request state nats_client request_json =
  let request =
    match
      risk_check_request_of_yojson (Yojson.Safe.from_string request_json)
    with
    | Ok req -> req
    | Error _ -> failwith "Invalid risk check request"
  in
  ...
  let side =
    match request.side with "buy" -> Types.Buy | _ -> Types.Sell
  in
  (* Create a temporary order for checking *)
  let order =
    Order.create_order ~id:order_id ~client_id:account_id ~instrument_id
      ~side ~order_type:Types.Market (* Assume market for risk check *)
      ~quantity ~timestamp:(Unix.gettimeofday ())
  in
  ...
  let result =
    Checks.run_pre_trade_checks ~limits ~portfolio ~order ~market_price
  in
  let passed =
    match result with
    | Checks.Passed -> true
  ...
  let* () =
    Nats_client.publish_string nats_client ~subject:"risk.events"
      response_json
  in
  Lwt.return (Ok result)

let handle_position_update state request_json =
  let request =
    match
      position_update_request_of_yojson
        (Yojson.Safe.from_string request_json)
    with
    | Ok req -> req
    | Error _ -> failwith "Invalid position update request"
  in
  let account_id = request.account_id in
  ...
  State.update_position state account_id ~instrument_id ~quantity ~side
    ~price ;
  Lwt.return_unit

type market_data_update = {instrument_id: string; bid: float; ask: float}
[@@deriving yojson]
...
let handle_market_data_update state update_json =
  let update =
    match
      market_data_update_of_yojson (Yojson.Safe.from_string update_json)
    with
    | Ok upd -> upd
    | Error _ -> failwith "Invalid market data update"
  in
...
let handle_limits_update state nats_client update_json =
  let update =
    match limits_update_of_yojson (Yojson.Safe.from_string update_json) with
    | Ok upd -> upd
    | Error _ -> failwith "Invalid limits update"
  in
  ...
  in
  State.update_limits state account_id new_limits ;
  let response_json =
    Yojson.Safe.to_string
      (`Assoc
  ...
  in
  let* () =
    Nats_client.publish_string nats_client ~subject:"risk.limits.updated"
      response_json
  in
  Lwt.return_unit
