...
type rsi = {value: float; oversold: bool; overbought: bool; divergence: bool}
[@@deriving show, yojson]
...
type stochastic =
  {k_percent: float; d_percent: float; oversold: bool; overbought: bool}
[@@deriving show, yojson]

type volume_profile =
  {volume_weighted_price: float; volume_ratio: float; above_average: bool}
[@@deriving show, yojson]
...
type market_phase = [`Markup | `Markdown | `Sideways | `Breakout | `Reversal]
[@@deriving show, yojson]
...
let calculate_rsi prices ~period =
  if List.length prices < period + 1 then None
    ...
    let price_changes =
      List.zip_exn (List.tl_exn prices) prices
      |> List.map ~f:(fun (curr, prev) -> curr -. prev)
    in
    let gains, losses =
      List.partition_tf price_changes ~f:(fun change -> Float.(change > 0.0))
    in
    let avg_gain =
      List.fold gains ~init:0.0 ~f:( +. ) /. Float.of_int (List.length gains)
    in
    let avg_loss =
      List.fold losses ~init:0.0 ~f:(fun acc loss -> acc +. Float.abs loss)
      /. Float.of_int (List.length losses)
    in
    if Float.(avg_loss = 0.0) then
      Some
        {value= 100.0; oversold= false; overbought= true; divergence= false}
    else
...
let calculate_bollinger_bands prices ~period ~std_dev_multiplier =
  ...
  | Some middle ->
      let recent = List.take prices period in
      let variance =
        List.fold recent ~init:0.0 ~f:(fun acc price ->
      ...
      Some {upper; middle; lower; bandwidth; percent_b}
...
let calculate_macd prices ~fast_period ~slow_period ~signal_period =
  ...
  match
    ( calculate_ema prices ~period:fast_period ~alpha:alpha_fast
    , calculate_ema prices ~period:slow_period ~alpha:alpha_slow )
  with
  | Some ema_fast, Some ema_slow ->
      ...
      Some
        ...
        ; histogram
        ; bullish_crossover=
            Float.(macd_line > signal_line && histogram > 0.0)
        ; bearish_crossover=
            Float.(macd_line < signal_line && histogram < 0.0) }
  | _ -> None
...
let calculate_volatility prices ~period =
  if List.length prices < period then None
    ...
    let recent = List.take prices period in
    let returns =
      List.zip_exn (List.tl_exn recent) recent
      |> List.map ~f:(fun (curr, prev) -> Float.log (curr /. prev))
    in
    let mean_return =
      List.fold returns ~init:0.0 ~f:( +. )
      /. Float.of_int (List.length returns)
    in
    let variance =
      List.fold returns ~init:0.0 ~f:(fun acc ret ->
          acc +. ((ret -. mean_return) *. (ret -. mean_return)) )
      /. Float.of_int (List.length returns - 1)
    in
    Some (Float.sqrt variance *. Float.sqrt 252.0)
...
let calculate_microstructure ~bid ~ask ~volume ~trades =
  ...
  let effective_spread = bid_ask_spread /. mid_price *. 10000.0 in
  (* in bps *)
  let avg_trade_size =
    if trades > 0 then volume /. Float.of_int trades else 0.0
  in
  let price_impact = effective_spread *. Float.log (1.0 +. avg_trade_size) in
  let market_depth = (bid +. ask) *. volume /. 2.0 in
  {bid_ask_spread; effective_spread; price_impact; market_depth}
...
let calculate_trend_strength prices ~period =
  if List.length prices < period then None
    ...
    let sum_absolute_changes =
      List.zip_exn (List.tl_exn recent) recent
      |> List.fold ~init:0.0 ~f:(fun acc (curr, prev) ->
             acc +. Float.abs (curr -. prev) )
    in
...
let detect_market_phase prices ~short_period ~long_period =
  match
    ( calculate_sma prices ~period:short_period
    , calculate_sma prices ~period:long_period )
  with
  | Some short_ma, Some long_ma ->
      ...
      if Float.(price_vs_short > 0.02 && price_vs_long > 0.01) then
        Some `Markup
      else if Float.(price_vs_short < -0.02 && price_vs_long < -0.01) then
        Some `Markdown
...
let generate_analytics ~instrument_id ~prices ~volumes ~timestamp
    ?(bid = None) ?(ask = None) () =
  let current_price = List.hd_exn prices in
  { instrument_id
  ; timestamp
  ...
  ; (* Technical Indicators *)
    bollinger_20_2=
      calculate_bollinger_bands prices ~period:20 ~std_dev_multiplier:2.0
  ; atr_14= None
  ; (* Would need high/low data *)
  ...
  ; (* Simplified *)
    ...
    microstructure=
      ...
      | Some b, Some a ->
          Some
            (calculate_microstructure ~bid:b ~ask:a
               ~volume:(List.hd_exn volumes) ~trades:100 )
      | _ -> None )
  ; (* Statistical Analysis *)
    statistics=
      ( match calculate_volatility prices ~period:30 with
      | Some vol ->
          Some
