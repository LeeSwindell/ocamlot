...
type depth_level = {price: float; quantity: float; order_count: int}
[@@deriving show, yojson]
...
type price_model =
  ...
  | JumpDiffusion of
      {volatility: float; jump_intensity: float; jump_size: float}
[@@deriving show]
...
(* Global market state *)
let market_state : (string, float * float list) Hashtbl.t =
...
let generate_next_price current_price model ~dt =
  ...
  | MeanReverting {mean; reversion_speed; volatility} ->
      let dw = generate_normal_random random_state *. Float.sqrt dt in
      current_price
      +. (reversion_speed *. (mean -. current_price) *. dt)
      +. (volatility *. dw)
  | JumpDiffusion {volatility; jump_intensity; jump_size} ->
      let dw = generate_normal_random random_state *. Float.sqrt dt in
      let diffusion = current_price *. volatility *. dw in
      let jump =
        if Float.(Random.State.float random_state 1.0 < jump_intensity *. dt)
        then
...
let generate_volume profile =
  let base_volume =
    profile.min_quantity
    +. Random.State.float random_state
         (profile.max_quantity -. profile.min_quantity)
  in
...
let generate_market_tick profile =
  ...
  Hashtbl.set market_state ~key:profile.symbol
    ~data:(rounded_price, new_history) ;
  let bid, ask =
    generate_bid_ask_spread ~mid_price:rounded_price
...
let generate_tick_data profile ~side =
  ...
  in
  let bid, ask =
    generate_bid_ask_spread ~mid_price:current_price
      ~spread_bps:profile.spread_bps
  in
  let price = match side with `Buy -> ask | `Sell -> bid in
...
let generate_market_depth profile ~levels =
  ...
  let generate_depth_side ~is_bid ~base_price ~levels =
    ...
    |> List.map ~f:(fun level ->
           ...
           let price =
             if is_bid then base_price -. price_offset
             else base_price +. price_offset
           in
           let quantity =
             generate_volume profile *. (1.0 /. Float.of_int level)
           in
...
let default_profiles =
  ...
  ; { symbol= "GOOGL"
    ...
    ; model=
        MeanReverting {mean= 2800.0; reversion_speed= 0.1; volatility= 45.0}
    ; spread_bps= 8.0
    ; min_quantity= 10.0
    ; max_quantity= 100.0
    ; tick_size= 0.01 }
  ...
  ; { symbol= "TSLA"
    ...
    ; model=
        JumpDiffusion
          {volatility= 0.45; jump_intensity= 0.02; jump_size= 0.05}
    ; spread_bps= 15.0
    ; min_quantity= 50.0
    ; max_quantity= 500.0
    ; tick_size= 0.01 }
...
let set_random_seed seed = Random.State.int random_state seed |> ignore
...
let get_current_price symbol =
  Hashtbl.find market_state symbol |> Option.map ~f:fst
...
let generate_tick_stream profiles ~count =
  ...
  |> List.concat_map ~f:(fun _ ->
         List.map profiles ~f:(fun profile ->
             let side =
               if Random.State.bool random_state then `Buy else `Sell
             in
...
let create_replay_data profiles ~duration_seconds ~tick_frequency =
  ...
  let total_ticks =
    Int.of_float (Float.of_int duration_seconds *. tick_frequency)
  in
  List.range 0 total_ticks
  |> List.concat_map ~f:(fun tick ->
         let timestamp =
           Float.of_int (Int.of_float (Unix.gettimeofday ()))
           +. (Float.of_int tick *. tick_interval)
         in
         profiles
...
let calculate_feed_stats profiles ~runtime_seconds =
  let price_ranges =
    List.map profiles ~f:(fun profile ->
        ...
        let min_price =
          List.min_elt history ~compare:Float.compare
          |> Option.value ~default:0.0
        in
        let max_price =
          List.max_elt history ~compare:Float.compare
          |> Option.value ~default:0.0
        in
        (profile.symbol, min_price, max_price) )
  in
  let total_volume =
    List.fold profiles ~init:0.0 ~f:(fun acc profile ->
