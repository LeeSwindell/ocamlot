...
let update_bar bar tick =
  let open_price =
    if Float.(bar.open_price = 0.0) then tick.price else bar.open_price
  in
  { open_price
...
let calculate_vwap ticks =
  ...
  in
  if Float.(total_volume > 0.0) then total_value /. total_volume else 0.0

let calculate_twap prices ~start_time ~end_time =
  let filtered =
    List.filter prices ~f:(fun (_, timestamp) ->
  ...
  | prices ->
      let sum =
        List.fold prices ~init:0.0 ~f:(fun acc (price, _) -> acc +. price)
      in
      sum /. Float.of_int (List.length prices)

let calculate_spread ~bid ~ask = ask -. bid

let calculate_mid_price ~bid ~ask = (bid +. ask) /. 2.0
...
let calculate_weighted_mid ~bid ~bid_size ~ask ~ask_size =
  ...
  else calculate_mid_price ~bid ~ask
