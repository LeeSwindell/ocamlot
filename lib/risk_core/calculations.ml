...
type margin_type = Cash | RegT | Portfolio [@@deriving show]

type margin_requirements =
  {initial_margin: float; maintenance_margin: float; available_margin: float}
[@@deriving show]
...
let calculate_var ~positions ~confidence_level ~holding_period =
  ...
  let portfolio_value =
    List.fold positions ~init:0.0 ~f:(fun acc (_, value, _) -> acc +. value)
  in
  let volatility = 0.02 in
  (* 2% daily volatility assumption *)
  let z_score =
    match confidence_level with 0.95 -> 1.645 | 0.99 -> 2.326 | _ -> 1.645
  in
...
let calculate_exposure ~positions =
  let _long_exposure, _short_exposure =
    List.fold positions ~init:(0.0, 0.0)
      ~f:(fun (long, short) (_, value, qty) ->
        if Float.(qty > 0.0) then (long +. value, short)
        else (long, short +. Float.abs value) )
  in
  {| 
...
let calculate_buying_power ~cash_balance ~margin_type ~current_positions =
  let position_value =
    List.fold current_positions ~init:0.0 ~f:(fun acc (_, value, _) ->
        acc +. Float.abs value )
  in
  match margin_type with
  | Cash -> cash_balance
