...
type transition_error =
  [`InvalidTransition of string | `InsufficientQuantity of float * float]
[@@deriving show]

type side_effect =
  | PublishEvent of Events.order_event
  | UpdatePosition of
      {instrument_id: string; quantity: float; side: Types.side}
...
let transition_to_filled order ~fill_qty ~fill_price ~timestamp =
  ...
  | New | PartiallyFilled _ ->
      ...
      if Float.(total_filled >= order.quantity) then
        ...
        let effects =
          [ PublishEvent
              (OrderFilled
                 {order_id= order.id; fill_qty; fill_price; timestamp} )
          ; UpdatePosition
        ...
        let updated_order =
          { order with
            status=
              PartiallyFilled
                {filled_qty= total_filled; avg_price= fill_price}
          ; updated_at= timestamp }
        in
        let effects =
          [ PublishEvent
  ...
  | Cancelled | Rejected _ | Filled _ ->
      Error
        (`InvalidTransition
           "Cannot fill a cancelled, rejected, or already filled order" )

let transition_to_cancelled order ~timestamp =
  match order.Order.status with
  | New | PartiallyFilled _ ->
      let updated_order =
        {order with status= Cancelled; updated_at= timestamp}
      in
      let effects =
...
let transition_to_rejected order ~reason ~timestamp =
  ...
  | New ->
      let updated_order =
        {order with status= Rejected reason; updated_at= timestamp}
      in
      let effects =
        [PublishEvent (OrderRejected {order_id= order.id; reason; timestamp})]
      in
      Ok (updated_order, effects)
  | _ -> Error (`InvalidTransition "Can only reject new orders")
...
let apply_amendment order ~new_quantity ~new_price ~timestamp =
  ...
  | New | PartiallyFilled _ ->
      ...
      if Float.(new_quantity < min_qty) then
        ...
        let new_order_type =
          match (order.order_type, new_price) with
  ...
  | _ ->
      Error
        (`InvalidTransition
           "Cannot amend cancelled, rejected, or filled orders" )
