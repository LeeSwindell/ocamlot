...
let validate_quantity ~max_quantity order =
  if Float.(order.Order.quantity <= max_quantity) then Ok order
  else Error (`MaxQuantityExceeded (order.quantity, max_quantity))

let validate_symbol ~valid_symbols order =
  if List.mem valid_symbols order.Order.instrument_id ~equal:String.equal
  then Ok order
  else Error (`InvalidSymbol order.instrument_id)
...
let validate_price_bounds ~min_price ~max_price order =
  ...
  | Types.StopLimit (stop, limit) ->
      if
        Float.(
          stop >= min_price && stop <= max_price && limit >= min_price
          && limit <= max_price )
      then Ok order
...
let validation_pipeline rules order =
  ...
  >>= validate_price_bounds ~min_price:rules.min_price
