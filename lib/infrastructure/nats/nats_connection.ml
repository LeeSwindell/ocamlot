...
let connect config =
  try%lwt
    ...
    in
    Lwt.return
      {conn with protocol_state= Connected; server_info= Some server_info}
  with Unix.Unix_error (errno, func, arg) ->
    let error_msg =
      Printf.sprintf "%s(%s): %s" func arg (Unix.error_message errno)
    in
...
let disconnect conn =
  ...
  let close_channels () =
    ...
    | Some ic ->
        ...
        Lwt.catch (fun () -> Lwt_io.close ic) (fun _ -> Lwt.return ())
    | None -> (
      (* If no input channel, try output channel *)
      match conn.output_channel with
      | Some oc ->
          Lwt.catch (fun () -> Lwt_io.close oc) (fun _ -> Lwt.return ())
      | None -> Lwt.return () )
  in
...
let is_connected conn =
  match conn.protocol_state with Connected -> true | _ -> false

let publish conn ~subject ?reply_to payload =
  if not (is_connected conn) then Lwt.fail Connection_closed
  else
...
let subscribe conn ~subject ~callback =
  if not (is_connected conn) then Lwt.fail Connection_closed
    ...
    let conn'' =
      { conn' with
        subscriptions=
          Map.add_exn conn'.subscriptions ~key:sid ~data:(subject, callback)
      }
    in
...
let unsubscribe conn ~sid =
  if not (is_connected conn) then Lwt.fail Connection_closed
    ...
    let conn' =
      {conn with subscriptions= Map.remove conn.subscriptions sid}
    in
...
let process_message_line conn line =
  if String.is_prefix line ~prefix:"MSG " then
    ...
    String.is_prefix line ~prefix:"+OK"
    || String.is_prefix line ~prefix:"-ERR"
  then
