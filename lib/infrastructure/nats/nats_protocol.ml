...
type message =
  {subject: string; sid: string; reply_to: string option; payload: bytes}
...
let parse_info_message data =
  ...
  with Yojson.Json_error msg ->
    raise (Protocol_error ("JSON parse error: " ^ msg))
...
let build_pub_message ~subject ?reply_to payload =
  let reply_part = match reply_to with Some r -> " " ^ r | None -> "" in
...
let parse_message_line line =
  ...
  | _ -> raise (Invalid_message ("Invalid MSG line: " ^ line))

(* Protocol state machine *)
type protocol_state = WaitingInfo | Connected | Closed
