...
module Client = struct
  ...
  type subscription = {client: t; sid: string}
  ...
  let create ?(config = Nats_connection.default_config) () =
    {conn= None; message_loop= None; config}
  ...
  let disconnect t =
    (* Cancel message loop *)
    (match t.message_loop with Some loop -> Lwt.cancel loop | None -> ()) ;
  ...
  let publish t ~subject ?reply_to payload =
    ...
    | Some conn ->
        if not (Nats_connection.is_connected conn) then
          ...
          let* conn' =
            Nats_connection.publish conn ~subject ?reply_to payload
          in
          t.conn <- Some conn' ;
          Lwt.return_unit
  ...
  let subscribe t ~subject ~callback =
    ...
    | Some conn ->
        if not (Nats_connection.is_connected conn) then
          ...
          let* sid, conn' =
            Nats_connection.subscribe conn ~subject ~callback
          in
          t.conn <- Some conn' ;
  ...
  let unsubscribe subscription =
    ...
    | Some conn ->
        if not (Nats_connection.is_connected conn) then
          ...
          let* conn' =
            Nats_connection.unsubscribe conn ~sid:subscription.sid
          in
          t.conn <- Some conn' ;
          Lwt.return_unit

  let get_server_info t =
    match t.conn with Some conn -> conn.server_info | None -> None

  let get_connection_config t = t.config
  ...
  let request t ~subject ~payload ~timeout =
    ...
    in
    let* reply_sub =
      subscribe t ~subject:reply_subject ~callback:reply_handler
    in
