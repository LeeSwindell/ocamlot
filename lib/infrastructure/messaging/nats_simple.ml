...
(* Connection state *)
type connection = {url: string; connected: bool ref; id: int}

type subscription =
  {subject: string; connection: connection; active: bool ref; id: int}
...
let connect url =
  ...
  let conn = {url; connected= ref true; id= !connection_counter} in
...
let close conn =
  ...
  active_connections :=
    List.filter !active_connections ~f:(fun c -> not (phys_equal c conn)) ;
  Stdio.printf "NATS [SIMPLE]: Closed connection %d\n" conn.id

let is_connected conn = !(conn.connected)

(* Publishing *)
let publish conn subject data =
  if not (is_connected conn) then raise (Nats_error "Connection is closed")
  else (
...
let subscribe conn subject =
  if not (is_connected conn) then raise (Nats_error "Connection is closed")
    ...
    let sub =
      {subject; connection= conn; active= ref true; id= !subscription_counter}
    in
...
let unsubscribe sub =
  ...
  active_subscriptions :=
    List.filter !active_subscriptions ~f:(fun s -> not (phys_equal s sub)) ;
  Stdio.printf "NATS [SIMPLE]: Unsubscribed from %s (sub id: %d)\n"
    sub.subject sub.id

(* Utility functions *)
let get_connection_count () = List.length !active_connections

let get_subscription_count () = List.length !active_subscriptions
