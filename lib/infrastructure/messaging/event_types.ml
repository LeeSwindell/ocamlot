...
type order_event_payload =
  ...
  | OrderValidated of
      {order_id: Types.order_id; validated_at: Types.timestamp}
  | OrderRejected of
      {order_id: Types.order_id; reason: string; rejected_at: Types.timestamp}
  | OrderFilled of
      { order_id: Types.order_id
      ; fill_id: string
  ...
  | OrderCancelled of
      {order_id: Types.order_id; cancelled_at: Types.timestamp}
[@@deriving show]
...
type system_event_payload =
  | SystemStarted of
      {component: string; version: string; timestamp: Types.timestamp}
  | SystemStopped of {component: string; timestamp: Types.timestamp}
  | SystemAlert of
...
let get_subject_for_event = function
  | OrderEvent (OrderSubmitted {order_id; _}) ->
      "orders.submitted." ^ order_id
  | OrderEvent (OrderValidated {order_id; _}) ->
      "orders.validated." ^ order_id
  | OrderEvent (OrderRejected {order_id; _}) -> "orders.rejected." ^ order_id
  | OrderEvent (OrderFilled {order_id; _}) -> "orders.filled." ^ order_id
  | OrderEvent (OrderCancelled {order_id; _}) ->
      "orders.cancelled." ^ order_id
  | MarketEvent (PriceUpdate {instrument_id; _}) ->
      "market.prices." ^ instrument_id
  | MarketEvent (TradeExecuted {instrument_id; _}) ->
      "market.trades." ^ instrument_id
  | MarketEvent (MarketStatusChange {instrument_id; _}) ->
      "market.status." ^ instrument_id
  | RiskEvent (RiskCheckPassed {order_id; _}) ->
      "risk.checks.passed." ^ order_id
  | RiskEvent (RiskCheckFailed {order_id; _}) ->
      "risk.checks.failed." ^ order_id
  | RiskEvent (RiskLimitExceeded {client_id; _}) ->
      "risk.limits." ^ client_id
  | SystemEvent (SystemStarted {component; _}) ->
      "system.lifecycle." ^ component
  | SystemEvent (SystemStopped {component; _}) ->
      "system.lifecycle." ^ component
  | SystemEvent (SystemAlert {component; level; _}) ->
      "system.alerts." ^ level ^ "." ^ component
