...
let connect
  ...
  try
    ...
    log_info
      (Printf.sprintf "Connected to NATS server: %s:%d" config.nats_host
         config.nats_port ) ;
    Lwt.return (Ok ())
  with
...
let publish_event (event : event_payload event) =
  let* () = Lwt.return () in
  match !connection_state with
  | None -> Lwt.return (Error "Not connected to NATS server")
  | Some state -> (
    match state.nats_client with
    | None -> Lwt.return (Error "No active NATS connection")
    | Some client -> (
      try
        let serialized_event = serialize event in
        let* () =
          Nats.publish_string client ~subject:event.subject serialized_event
        in
        log_debug
          (Printf.sprintf "Published event to subject %s" event.subject) ;
        Lwt.return (Ok ())
      with
      | Nats.Client_error msg | Nats.Protocol_error msg ->
...
let subscribe subject handler =
  ...
  | Some state -> (
    ...
    | Some client -> (
      try
        let message_handler _msg data =
          ...
          | Deserialization_error err ->
              log_error
                (Printf.sprintf "Failed to deserialize message: %s" err) ;
              Lwt.return ()
          | exn ->
              log_error
                (Printf.sprintf "Error handling message: %s"
                   (Exn.to_string exn) ) ;
              Lwt.return ()
        in
        let* nats_sub =
          Nats.subscribe_string client ~subject ~callback:message_handler
        in
        let sub_info = {subject; nats_sub; handler; active= true} in
      ...
      | Nats.Client_error msg | Nats.Protocol_error msg ->
          log_error
            (Printf.sprintf "Failed to subscribe to %s: %s" subject msg) ;
          Lwt.return (Error msg)
      | exn ->
          let error_msg =
            "Unexpected subscription error: " ^ Exn.to_string exn
          in
          log_error (Printf.sprintf "%s" error_msg) ;
          Lwt.return (Error error_msg) ) )

(* Convenience functions for specific event types *)
let publish_order_event payload = publish_payload (OrderEvent payload)

let publish_market_event payload = publish_payload (MarketEvent payload)

let publish_risk_event payload = publish_payload (RiskEvent payload)

let publish_system_event payload = publish_payload (SystemEvent payload)
...
let get_connection_status () =
  match !connection_state with
  | None -> "disconnected"
  | Some state -> if state.is_connected then "connected" else "disconnected"
