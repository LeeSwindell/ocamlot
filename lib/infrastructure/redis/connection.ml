...
(* let show_connection_error = function | No_addresses host -> Printf.sprintf
   "No_addresses(%s)" host | Connection_failed exn -> Printf.sprintf
   "Connection_failed(%s)" (Printexc.to_string exn) | IO_error exn ->
   Printf.sprintf "IO_error(%s)" (Printexc.to_string exn) *)
...
let connect host port =
  Lwt.catch
    ...
    (fun exn -> Lwt.return_error (Connection_failed exn))
...
let with_connection host port f =
  ...
  | Ok handle -> Lwt.finalize (fun () -> f handle) (fun () -> close handle)
