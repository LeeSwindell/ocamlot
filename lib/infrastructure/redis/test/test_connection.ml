...
let create_mock_redis_server port =
    ...
    let rec handle_commands () =
      if not !server_running then Lwt.return_unit
        ...
        | Ok (Array (Some [BulkString (Some "PING")])) ->
            ...
            let* () =
              Lwt_io.write oc (Resp3.serialize_resp3 (SimpleString "PONG"))
            in
        ...
        | Ok (Array (Some [BulkString (Some "UNKNOWN")])) ->
            let* () =
              Lwt_io.write oc
                (Resp3.serialize_resp3 (SimpleError "ERR unknown command"))
            in
            let* () = Lwt_io.flush oc in
            handle_commands ()
        | Ok _ ->
            let* () =
              Lwt_io.write oc
                (Resp3.serialize_resp3 (SimpleError "ERR invalid command"))
            in
            let* () = Lwt_io.flush oc in
            handle_commands ()
        | Error _ ->
            ...
            Lwt.return_unit
    in
    Lwt.finalize handle_commands (fun () ->
  ...
  let* () =
    Lwt_unix.bind server_socket (ADDR_INET (Unix.inet_addr_loopback, port))
  in
...
let connection_error_testable =
  Alcotest.testable
    ...
    (fun e1 e2 ->
      Connection.show_connection_error e1
      = Connection.show_connection_error e2 )
...
let with_mock_server port f =
  ...
  Lwt.finalize
    ...
    (fun () ->
      ...
      Lwt.catch (fun () -> Lwt_unix.close server) (fun _ -> Lwt.return_unit) )
...
let test_ping_command () =
  with_mock_server 7778 (fun () ->
      ...
      match result with
      | Ok handle ->
          (* Send PING - the Connection module should handle RESP
             formatting *)
          let* send_result =
            Connection.send_command handle
              (Resp3.serialize_resp3 (Array (Some [BulkString (Some "PING")])))
          in
          let* () =
          ...
          let* () =
            ...
            | Error e ->
                let* () = Connection.close handle in
                Alcotest.fail
                  (Printf.sprintf "Receive failed: %s"
                     (Connection.show_connection_error e) )
            | Ok (Resp3.SimpleString "PONG") -> Lwt.return_unit
            | Ok other ->
                let* () = Connection.close handle in
                Alcotest.fail
                  (Printf.sprintf "Expected PONG, got: %s"
          ...
          let* () =
            ...
            | Ok _ ->
                if elapsed > 0.1 then
                  let* () = Connection.close handle in
                  Alcotest.fail
                    (Printf.sprintf "Response too slow: %f seconds" elapsed)
...
let test_unknown_command () =
  with_mock_server 7780 (fun () ->
      ...
      | Ok handle ->
          ...
          let* send_result =
            Connection.send_command handle
              (Resp3.serialize_resp3
                 (Array (Some [BulkString (Some "UNKNOWN")])) )
          in
          let* () =
            match send_result with
            | Error e ->
          ...
          let* () =
            match resp with
            | Ok (Resp3.SimpleError msg)
              when String.length msg >= 3 && String.sub msg 0 3 = "ERR" ->
                Lwt.return_unit
            | Ok other ->
                let* () = Connection.close handle in
                Alcotest.fail
...
(* If you want to add more test suites in the same style as your parser
   tests *)
