...
let null_parser_tests = [test "null value" "_\r\n" (Ok Null)]
...
let push_parser_tests =
  ...
  ; test "push with multiple data"
      ">4\r\n+monitor\r\n:123\r\n+client\r\n+GET key\r\n"
      (Ok
         (Push
            { kind= "monitor"
            ; data=
                [Integer 123L; SimpleString "client"; SimpleString "GET key"]
            } ) )
  ; test "push with bulk string kind"
      ">3\r\n$7\r\nmessage\r\n+channel\r\n+data\r\n"
      (Ok
         (Push
            { kind= "message"
...
let invalid_parser_tests =
  [ (* Empty message *)
    test "empty message" ""
      (Error "empty: Invalid RESP format - Empty message")
  ; (* Invalid prefix *)
  ...
  ; (* Simple String (+) failures *)
    test "+ no CRLF" "+OK"
      (Error
         "Incomplete message - expected more data (may be missing a prefix)"
      )
  ; test "+ only LF" "+OK\n"
      (Error
         "Incomplete message - expected more data (may be missing a prefix)"
  ...
  ; test ": invalid integer no CRLF" ":abc"
      (Error
         "Incomplete message - expected more data (may be missing a prefix)"
      )
  ; (* Bulk String ($) failures *)
    test "$ no CRLF length" "$5"
      (Error
         "Incomplete message - expected more data (may be missing a prefix)"
      )
  ...
  ; test "* incomplete array" "*2\r\n+OK\r\n"
      (Error
         "array > array elements > empty: Invalid RESP format - Empty \
          message" )
  ; (* Null (_) failures *)
    test "_ no CRLF" "_"
      (Error
  ...
  ; test "# only CR true" "#t\r"
      (Error
         "Incomplete message - expected more data (may be missing a prefix)"
      )
  ; test "# no CRLF false" "#f"
      (Error
         "Incomplete message - expected more data (may be missing a prefix)"
      )
  ...
  ; (* This should fail for the invalid bool before the CRLF *)
    test "# invalid boolean no CRLF" "#x"
      (Error "boolean: Invalid boolean value: x")
  ; (* Double (,) failures *)
    test ", no CRLF" ",3.14"
      (Error
  ...
  ; test ", invalid double no CRLF" ",abc"
      (Error
         "Incomplete message - expected more data (may be missing a prefix)"
      )
  ; (* Big Number (() failures *)
    test "( no CRLF" "(123456789"
      (Error
         "Incomplete message - expected more data (may be missing a prefix)"
      )
  ...
  ; test "% incomplete map" "%1\r\n+key\r\n"
      (Error
         "map > map key-value pairs > empty: Invalid RESP format - Empty \
          message" )
  ; (* Set (~) failures *)
    test "~ no CRLF" "~3"
      (Error
  ...
  ; test "~ incomplete set" "~2\r\n+elem1\r\n"
      (Error
         "set > set elements > empty: Invalid RESP format - Empty message" )
  ; (* Push (>) failures *)
    test "> no CRLF" ">2"
      (Error
         "Incomplete message - expected more data (may be missing a prefix)"
  ...
  ; test "| missing value" "|1\r\n+key\r\n+val\r\n"
      (Error
         "attribute > attribute value > empty: Invalid RESP format - Empty \
          message" )
  ; (* Edge cases *)
    test "only CRLF" "\r\n"
...
let buffered_parser_tests =
  [ test_case "parse complete message in one chunk" `Quick (fun () ->
        ...
        check
          (result resp_value_testable string)
          "complete message" (Ok (Array (Some [BulkString (Some "PING")])))
          res )
  ; test_case "parse message split across chunks" `Quick (fun () ->
        ...
        check
          (result resp_value_testable string)
          "split message" (Ok (Array (Some [BulkString (Some "PING")]))) res )
  ; test_case "parse with trailing data" `Quick (fun () ->
        ...
        check
          (result resp_value_testable string)
          "first message only" (Ok (SimpleString "PONG")) res )
  ; test_case "handle EOF mid-message" `Quick (fun () ->
        ...
        check
          (result resp_value_testable string)
          "incomplete message" (Error "Incomplete RESP message") res )
  ; test_case "byte-by-byte parsing" `Quick (fun () ->
        ...
        check
          (result resp_value_testable string)
          "byte by byte" (Ok (SimpleString "OK")) res )
  ; (* Test that buffered parser handles binary data correctly *)
    test_case "binary data with nulls" `Quick (fun () ->
        ...
        check
          (result resp_value_testable string)
          "binary with nulls" (Ok (BulkString (Some binary))) res ) ]
