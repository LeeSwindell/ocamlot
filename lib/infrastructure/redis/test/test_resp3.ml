...
let sync_to_lwt test_list =
  List.map
    (fun (name, speed, test_fn) ->
      (name, speed, fun _switch -> test_fn () ; Lwt.return_unit) )
    test_list

let () =
  Lwt_main.run
    (Alcotest_lwt.run "RESP3 and Connection Tests"
       [ (* Parser tests *)
       ...
       ; (* Serializer tests *)
         ( "serializer: simple string"
         , sync_to_lwt simple_string_serializer_tests )
       ; ( "serializer: simple error"
         , sync_to_lwt simple_error_serializer_tests )
       ...
       ; ( "serializer: verbatim string"
         , sync_to_lwt verbatim_string_serializer_tests )
       ...
       ; (* Comprehensive command category tests *)
         ( "commands: connection & server"
         , sync_to_lwt connection_server_tests )
