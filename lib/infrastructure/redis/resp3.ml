...
type resp_value =
  ...
  | BigNumber of Z.t [@printer pp_z]
    (* (3492890328409238509324850943850943850\r\n *)
...
let parse_length =
  take_while1 is_length_char <* crlf >>| int_of_string <?> "length field"

(* Parse bulk string body after the $ *)
let parse_bulk_string_body =
  parse_length
  >>= fun len ->
  if len < 0 then return (BulkString None)
  else
...
let parse_array_body self =
  parse_length
  >>= fun num_items ->
  if num_items < 0 then return (Array None)
  else
    count num_items self <?> "array elements"
    >>| fun items -> Array (Some items)
...
let parse_verbatim_string_body =
  ...
  take 3 <?> "verbatim format"
  >>= fun format ->
  char ':' <?> "verbatim colon"
  >>= fun _ ->
  take (len - 4)
  <?> "verbatim string data" <* crlf
  >>| fun content -> VerbatimString {format; content}
...
let parse_attribute_body self =
  ...
  self <?> "attribute value" >>| fun value -> Attribute {attrs; value}

(* Parse set body after the ~ *)
let parse_set_body self =
  parse_length
  >>= fun len -> count len self <?> "set elements" >>| fun items -> Set items

(* Parse push body after the > *)
let parse_push_body self =
  parse_length
  >>= fun length ->
  if length < 2 then fail "Push must have at least kind and one element"
  else
    self <?> "push type"
    >>= fun first ->
  ...
  fix (fun self ->
      ...
      | Some c -> (
        ...
        | ':' ->
            ...
            <?> "integer"
        (* Null *)
        | '_' -> char '_' *> crlf *> return Null <?> "null"
        (* Boolean *)
        | '#' ->
            char '#' *> any_char
            >>= (function
        ...
        | '(' ->
            ...
            <?> "big number"
        (* Bulk string *)
        | '$' -> char '$' *> parse_bulk_string_body <?> "bulk string"
        (* Array *)
        | '*' -> char '*' *> parse_array_body self <?> "array"
        (* Bulk error *)
        | '!' -> char '!' *> parse_bulk_error_body <?> "bulk error"
        (* Verbatim string *)
        | '=' -> char '=' *> parse_verbatim_string_body <?> "verbatim string"
        (* Map *)
        | '%' -> char '%' *> parse_map_body self <?> "map"
        (* Attribute *)
        | '|' -> char '|' *> parse_attribute_body self <?> "attribute"
        (* Set *)
        | '~' -> char '~' *> parse_set_body self <?> "set"
        (* Push *)
        | '>' -> char '>' *> parse_push_body self <?> "push"
        (* Invalid prefix *)
        | c ->
            fail
              (Printf.sprintf "Invalid RESP format - Unknown prefix: %c" c)
            <?> "invalid prefix" ) )

let parse_resp_buffered (reader : unit -> string Lwt.t) :
    (resp_value, string) result Lwt.t =
...
let parse_resp_from_channel (input : Lwt_io.input_channel) :
    (resp_value, string) result Lwt.t =
...
let parse_string str =
  ...
  | Error msg
    when B.String.is_substring ~substring:"expected string \"\\r\\n\"" msg ->
      Error "Invalid line ending - expected CRLF (\\r\\n)"
  | Error msg -> Error msg

(* The construct below uses mutual recursion among the helper functions. This
   is needed since an Arry may need to be serialized for each element, which
   could contain maps, that in turn have their own elements, and so on. *)
...
and serialize_map pairs =
  ...
  ^ "\r\n"
  ^ String.concat ""
      ( List.fold_left (fun acc (k, v) -> acc @ [k; v]) [] pairs
      |> List.map serialize_resp3 )
...
and serialize_attribute attrs value =
  ...
  ^ "\r\n"
  ^ String.concat ""
      ( List.fold_left (fun acc (k, v) -> acc @ [k; v]) [] attrs
      |> List.map serialize_resp3 )
