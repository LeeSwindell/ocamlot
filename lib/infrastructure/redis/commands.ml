...
let format_timeout timeout =
  if Float.is_integer timeout then string_of_int (int_of_float timeout)
  else string_of_float timeout
...
let quit () = Array (Some [BulkString (Some "QUIT")])

(** SELECT index - Change the selected database *)
let select db_index =
  Array
    (Some
       [ BulkString (Some "SELECT")
       ; BulkString (Some (string_of_int db_index)) ] )
...
let expire key seconds =
  Array
    (Some
       [ BulkString (Some "EXPIRE")
       ; BulkString (Some key)
       ; BulkString (Some (string_of_int seconds)) ] )

(** EXPIREAT key timestamp - Set key expiration at timestamp *)
let expireat key timestamp =
  Array
    (Some
       [ BulkString (Some "EXPIREAT")
       ; BulkString (Some key)
       ; BulkString (Some (Int64.to_string timestamp)) ] )

(** TTL key - Get time to live *)
let ttl key = Array (Some [BulkString (Some "TTL"); BulkString (Some key)])
...
let rename key newkey =
  Array
    (Some
       [ BulkString (Some "RENAME")
       ; BulkString (Some key)
       ; BulkString (Some newkey) ] )

(** RENAMENX key newkey - Rename key if new key doesn't exist *)
let renamenx key newkey =
  Array
    (Some
       [ BulkString (Some "RENAMENX")
       ; BulkString (Some key)
       ; BulkString (Some newkey) ] )

(** DUMP key - Serialize key value *)
let dump key = Array (Some [BulkString (Some "DUMP"); BulkString (Some key)])

(** RESTORE key ttl serialized-value - Restore serialized key *)
let restore key ttl serialized_value ?replace () =
  let base_cmd =
    [ BulkString (Some "RESTORE")
    ; BulkString (Some key)
    ; BulkString (Some (string_of_int ttl))
    ; BulkString (Some serialized_value) ]
  in
  let cmd =
    match replace with
...
let scan cursor ?pattern ?count () =
  let base_cmd =
    [BulkString (Some "SCAN"); BulkString (Some (string_of_int cursor))]
  in
  let cmd_with_match =
    match pattern with
    | None -> base_cmd
    | Some p -> base_cmd @ [BulkString (Some "MATCH"); BulkString (Some p)]
  in
  let final_cmd =
    match count with
    | None -> cmd_with_match
    | Some c ->
        cmd_with_match
        @ [BulkString (Some "COUNT"); BulkString (Some (string_of_int c))]
  in
  Array (Some final_cmd)

(** RANDOMKEY - Return a random key *)
let randomkey () = Array (Some [BulkString (Some "RANDOMKEY")])

(** MOVE key db - Move key to another database *)
let move key db =
  Array
    (Some
       [ BulkString (Some "MOVE")
       ; BulkString (Some key)
       ; BulkString (Some (string_of_int db)) ] )
...
let get key = Array (Some [BulkString (Some "GET"); BulkString (Some key)])

(** SET key value [EX seconds] [PX milliseconds] [NX|XX] - Set string value *)
let set key value ?ex ?px ?nx ?xx () =
  let base_cmd =
    [BulkString (Some "SET"); BulkString (Some key); BulkString (Some value)]
  in
  let cmd_with_ex =
    match ex with
    | None -> base_cmd
    | Some seconds ->
        base_cmd
        @ [BulkString (Some "EX"); BulkString (Some (string_of_int seconds))]
  in
  let cmd_with_px =
    match px with
    | None -> cmd_with_ex
    | Some milliseconds ->
        cmd_with_ex
        @ [ BulkString (Some "PX")
          ; BulkString (Some (string_of_int milliseconds)) ]
  in
  let cmd_with_nx =
    match nx with
...
let mset key_values =
  let args =
    List.fold_right
      (fun (key, value) acc ->
        BulkString (Some key) :: BulkString (Some value) :: acc )
      key_values []
  in
...
let psetex key milliseconds value =
  Array
    (Some
       [ BulkString (Some "PSETEX")
       ; BulkString (Some key)
       ; BulkString (Some (string_of_int milliseconds))
       ; BulkString (Some value) ] )

(** INCR key - Increment integer value *)
let incr key = Array (Some [BulkString (Some "INCR"); BulkString (Some key)])
...
let getrange key start_pos end_pos =
  Array
    (Some
       [ BulkString (Some "GETRANGE")
       ; BulkString (Some key)
       ; BulkString (Some (string_of_int start_pos))
       ; BulkString (Some (string_of_int end_pos)) ] )

(** SETRANGE key offset value - Overwrite string at offset *)
let setrange key offset value =
  Array
    (Some
       [ BulkString (Some "SETRANGE")
       ; BulkString (Some key)
...
let hset key field_values =
  ...
  in
  let cmd =
    BulkString (Some "HSET") :: BulkString (Some key) :: field_value_args
  in
  Array (Some cmd)
...
let hmget key fields =
  ...
  Array
    (Some (BulkString (Some "HMGET") :: BulkString (Some key) :: field_args))
...
let hmset key field_values =
  ...
  in
  let cmd =
    BulkString (Some "HMSET") :: BulkString (Some key) :: field_value_args
  in
  Array (Some cmd)
...
let hlen key = Array (Some [BulkString (Some "HLEN"); BulkString (Some key)])

(** HDEL key field [field ...] - Delete hash fields *)
let hdel key fields =
  let field_args = List.map (fun f -> BulkString (Some f)) fields in
  Array
    (Some (BulkString (Some "HDEL") :: BulkString (Some key) :: field_args))

(** HEXISTS key field - Check if hash field exists *)
let hexists key field =
  Array
    (Some
       [ BulkString (Some "HEXISTS")
       ; BulkString (Some key)
       ; BulkString (Some field) ] )
...
let hscan key cursor ?pattern ?count () =
  let base_cmd =
    [ BulkString (Some "HSCAN")
    ; BulkString (Some key)
    ; BulkString (Some (string_of_int cursor)) ]
  in
  let cmd_with_match =
    match pattern with
    | None -> base_cmd
...
let lpush key values =
  ...
  Array
    (Some (BulkString (Some "LPUSH") :: BulkString (Some key) :: value_args))

(** RPUSH key value [value ...] - Push to list tail *)
let rpush key values =
  let value_args = List.map (fun v -> BulkString (Some v)) values in
  Array
    (Some (BulkString (Some "RPUSH") :: BulkString (Some key) :: value_args))

(** LPOP key - Pop from list head *)
let lpop key = Array (Some [BulkString (Some "LPOP"); BulkString (Some key)])

(** RPOP key - Pop from list tail *)
let rpop key = Array (Some [BulkString (Some "RPOP"); BulkString (Some key)])

(** LLEN key - Get list length *)
let llen key = Array (Some [BulkString (Some "LLEN"); BulkString (Some key)])

(** LRANGE key start stop - Get list range *)
let lrange key start_idx stop_idx =
  Array
    (Some
       [ BulkString (Some "LRANGE")
       ; BulkString (Some key)
       ; BulkString (Some (string_of_int start_idx))
       ; BulkString (Some (string_of_int stop_idx)) ] )

(** LTRIM key start stop - Trim list to range *)
let ltrim key start_idx stop_idx =
  Array
    (Some
       [ BulkString (Some "LTRIM")
       ; BulkString (Some key)
       ; BulkString (Some (string_of_int start_idx))
       ; BulkString (Some (string_of_int stop_idx)) ] )
...
let lindex key index =
  Array
    (Some
       [ BulkString (Some "LINDEX")
       ; BulkString (Some key)
       ; BulkString (Some (string_of_int index)) ] )

(** LSET key index value - Set list element by index *)
let lset key index value =
  Array
    (Some
       [ BulkString (Some "LSET")
       ; BulkString (Some key)
       ; BulkString (Some (string_of_int index))
       ; BulkString (Some value) ] )
...
let linsert key ~before pivot value =
  ...
  Array
    (Some
       [ BulkString (Some "LINSERT")
       ; BulkString (Some key)
       ; BulkString (Some direction)
       ; BulkString (Some pivot)
       ; BulkString (Some value) ] )
...
let blpop keys timeout =
  ...
  Array
    (Some
       ( (BulkString (Some "BLPOP") :: key_args)
       @ [BulkString (Some timeout_str)] ) )
...
let brpop keys timeout =
  ...
  Array
    (Some
       ( (BulkString (Some "BRPOP") :: key_args)
       @ [BulkString (Some timeout_str)] ) )
...
let brpoplpush source destination timeout =
  Array
    (Some
       [ BulkString (Some "BRPOPLPUSH")
       ; BulkString (Some source)
       ; BulkString (Some destination)
       ; BulkString (Some (format_timeout timeout)) ] )

(** RPOPLPUSH source destination - Pop and push *)
let rpoplpush source destination =
  Array
    (Some
       [ BulkString (Some "RPOPLPUSH")
       ; BulkString (Some source)
       ; BulkString (Some destination) ] )
...
let sadd key members =
  ...
  Array
    (Some (BulkString (Some "SADD") :: BulkString (Some key) :: member_args))

(** SREM key member [member ...] - Remove from set *)
let srem key members =
  let member_args = List.map (fun m -> BulkString (Some m)) members in
  Array
    (Some (BulkString (Some "SREM") :: BulkString (Some key) :: member_args))
...
let sismember key member =
  Array
    (Some
       [ BulkString (Some "SISMEMBER")
       ; BulkString (Some key)
       ; BulkString (Some member) ] )
...
let srandmember key ?count () =
  ...
  let final_cmd =
    match count with
    | None -> base_cmd
    | Some c -> base_cmd @ [BulkString (Some (string_of_int c))]
  in
  Array (Some final_cmd)
...
let sinterstore destination keys =
  ...
  Array
    (Some
       ( BulkString (Some "SINTERSTORE") :: BulkString (Some destination)
       :: key_args ) )
...
let sunionstore destination keys =
  ...
  Array
    (Some
       ( BulkString (Some "SUNIONSTORE") :: BulkString (Some destination)
       :: key_args ) )
...
let zadd key ?nx:_nx ?xx:_xx ?ch:_ch ?incr:_incr score_members =
  let score_member_args =
    List.fold_right
      (fun (score, member) acc ->
        BulkString (Some (format_numeric_arg score))
        :: BulkString (Some member) :: acc )
      score_members []
  in
  Array
    (Some
       ( BulkString (Some "ZADD") :: BulkString (Some key)
       :: score_member_args ) )
...
let zrem key members =
  let member_args = List.map (fun m -> BulkString (Some m)) members in
  Array
    (Some (BulkString (Some "ZREM") :: BulkString (Some key) :: member_args))
...
let zcount key min_score max_score =
  Array
    (Some
       [ BulkString (Some "ZCOUNT")
       ; BulkString (Some key)
       ; BulkString (Some (format_numeric_arg min_score))
       ; BulkString (Some (format_numeric_arg max_score)) ] )

(** ZSCORE key member - Get member score *)
let zscore key member =
  Array
    (Some
       [ BulkString (Some "ZSCORE")
       ; BulkString (Some key)
       ; BulkString (Some member) ] )
...
let zrange key start_idx stop_idx ?withscores () =
  let base_cmd =
    [ BulkString (Some "ZRANGE")
    ; BulkString (Some key)
    ; BulkString (Some (string_of_int start_idx))
    ; BulkString (Some (string_of_int stop_idx)) ]
  in
  let cmd =
    match withscores with
...
let zrevrangebyscore key max_score min_score ?withscores ?limit () =
  let base_cmd =
    [ BulkString (Some "ZREVRANGEBYSCORE")
    ; BulkString (Some key)
    ; BulkString (Some (format_numeric_arg max_score))
    ; BulkString (Some (format_numeric_arg min_score)) ]
  in
...
let zremrangebyrank key start_idx stop_idx =
  Array
    (Some
       [ BulkString (Some "ZREMRANGEBYRANK")
       ; BulkString (Some key)
       ; BulkString (Some (string_of_int start_idx))
       ; BulkString (Some (string_of_int stop_idx)) ] )
...
let zremrangebyscore key min_score max_score =
  Array
    (Some
       [ BulkString (Some "ZREMRANGEBYSCORE")
       ; BulkString (Some key)
       ; BulkString (Some (format_numeric_arg min_score))
       ; BulkString (Some (format_numeric_arg max_score)) ] )
...
let zinterstore destination keys ?weights ?aggregate () =
  ...
  let key_args = List.map (fun k -> BulkString (Some k)) keys in
  let base_cmd =
    BulkString (Some "ZINTERSTORE") :: BulkString (Some destination)
    :: BulkString (Some (string_of_int numkeys))
    :: key_args
  in
  let cmd_with_weights =
    match weights with
    | None -> base_cmd
    | Some w ->
        base_cmd
        @ [BulkString (Some "WEIGHTS")]
        @ List.map
            (fun weight -> BulkString (Some (format_numeric_arg weight)))
            w
  in
  let final_cmd =
    ...
    | Some agg ->
        ...
        in
        cmd_with_weights
        @ [BulkString (Some "AGGREGATE"); BulkString (Some agg_str)]
  in
  Array (Some final_cmd)
...
let zunionstore destination keys ?weights ?aggregate () =
  ...
  let key_args = List.map (fun k -> BulkString (Some k)) keys in
  let base_cmd =
    BulkString (Some "ZUNIONSTORE") :: BulkString (Some destination)
    :: BulkString (Some (string_of_int numkeys))
    :: key_args
  in
  let cmd_with_weights =
...
let bitcount key ?start ?end_pos () =
  let base_cmd = [BulkString (Some "BITCOUNT"); BulkString (Some key)] in
  let cmd_with_range =
    match (start, end_pos) with
    | Some s, Some e ->
        base_cmd
...
let bitfield key operations =
  ...
  let base_cmd = [BulkString (Some "BITFIELD"); BulkString (Some key)] in
...
let pfadd key elements =
  ...
  let base_cmd = [BulkString (Some "PFADD"); BulkString (Some key)] in
...
let pfmerge destkey sourcekeys =
  ...
  let base_cmd = [BulkString (Some "PFMERGE"); BulkString (Some destkey)] in
...
let xadd key id field_values ?nomkstream ?ref_handling ?trim_strategy ?limit
  ...
  let cmd_with_nomkstream =
    match nomkstream with
  ...
  let cmd_with_ref =
    ...
    | None -> cmd_with_nomkstream
  in
  (* Add trimming strategy if specified *)
  let cmd_with_trim =
    match trim_strategy with
    | Some (MaxLen (threshold, approximate)) ->
        ...
        let trim_with_limit =
          ...
          | Some lim ->
              trim_args
              @ [ BulkString (Some "LIMIT")
                ; BulkString (Some (string_of_int lim)) ]
          | None -> trim_args
        in
...
(** XGROUP CREATECONSUMER key group consumer - Create a consumer in the consumer group *)
let xgroup_createconsumer key groupname consumer =
  Array (Some 
    [BulkString (Some "XGROUP"); 
...
let multi () = Array (Some [BulkString (Some "MULTI")])

(** EXEC - Execute transaction *)
let exec () = Array (Some [BulkString (Some "EXEC")])

(** DISCARD - Discard transaction *)
let discard () = Array (Some [BulkString (Some "DISCARD")])
...
let unwatch () = Array (Some [BulkString (Some "UNWATCH")])
