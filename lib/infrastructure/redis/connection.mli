...
val connect :
  string -> int -> (connection_handle, connection_error) result Lwt.t
...
val send_command :
  connection_handle -> string -> (unit, connection_error) result Lwt.t
(** [send_command handle data] sends raw data to Redis.
    Automatically flushes the output buffer. *)

val read_line : connection_handle -> (string, connection_error) result Lwt.t
(** [read_line handle] reads until '\n' or '\r\n'.
    Used for reading RESP simple strings, errors, and integers. *)

val read_exact :
  connection_handle -> int -> (string, connection_error) result Lwt.t
(** [read_exact handle n] reads exactly [n] bytes.
    Used for reading RESP bulk string payloads. *)

val receive_resp :
  connection_handle -> (Resp3.resp_value, connection_error) result Lwt.t
(** [receive_resp handle] reads and parses a complete RESP3 value.
    Handles all RESP3 types including nested arrays. *)

(** {1 Pipeline Support} *)

(* Uncomment when implementing pipelining: (** [pipeline handle commands]
   sends multiple commands and collects all responses. More efficient than
   sequential send/receive for batch operations. *) val pipeline :
   connection_handle -> string list -> (Resp3.resp_value list,
   connection_error) result Lwt.t *)
