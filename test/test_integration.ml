...
let test_order_creation _switch () =
  let order =
    Order.create_order ~id:"order-1" ~client_id:"client-1"
      ~instrument_id:"AAPL" ~side:Types.Buy ~order_type:(Types.Limit 150.0)
      ~quantity:100.0 ~timestamp:(Unix.time ())
  in
...
let test_risk_limits _switch () =
  ...
  Alcotest.(check bool)
    "max order size positive" true
    (limits.max_order_size > 0.0) ;
  Lwt.return ()
...
let test_order_transitions _switch () =
  ...
  in
  let result =
    Transitions.transition_to_filled order ~fill_qty:50.0 ~fill_price:149.5
      ~timestamp:(Unix.time ())
  in
...
let tests =
  [ ("Order", [test_case "creation" `Quick test_order_creation])
  ; ( "Validation"
    , [test_case "order validation" `Quick test_order_validation] )
  ; ("Risk", [test_case "risk limits" `Quick test_risk_limits])
  ; ( "Transitions"
    , [test_case "order transitions" `Quick test_order_transitions] ) ]

let () = Lwt_main.run (run "OCamlot Integration Tests" tests)
