...
let _set_inner_html element html = element##.innerHTML := Js.string html
...
let format_price price = Printf.sprintf "%.2f" price

let format_timestamp timestamp =
  let date =
    new%js Js.date_fromTimeValue (Js.number_of_float (timestamp *. 1000.0))
  in
...
let update_market_data_display () =
  ...
  | Some tbody -> (
      ...
      let latest_data : web_market_data list =
        ClientState.get_latest_data 50
      in
      List.iteri
        (fun index (data : web_market_data) ->
          let trend_class =
            if index > 0 then
              try
                let prev_data = List.nth latest_data (index - 1) in
                if String.equal prev_data.instrument_id data.instrument_id
                then
                  get_price_trend_class prev_data.last_price data.last_price
                else "price-unchanged"
              with _ -> "price-unchanged"
            else "price-unchanged"
          in
      ...
      if !ClientState.auto_scroll then
        ...
        | None -> () )
  | None -> console_log "Market data table body not found"
...
let handle_websocket_message message_text =
  ...
  | Ok (ConflatedBar bar) ->
      console_log
        ( "Received conflated bar for " ^ bar.instrument_id ^ " ("
        ^ bar.interval ^ "): OHLCV="
        ^ Printf.sprintf "%.2f/%.2f/%.2f/%.2f/%.0f" bar.open_price
            bar.high_price bar.low_price bar.close_price bar.volume ) ;
      (* Convert conflated bar to market data tick for display *)
      let market_tick =
        { instrument_id= bar.instrument_id
        ; bid= bar.close_price -. 0.01
        ; (* Approximate bid from close price *)
          ask= bar.close_price +. 0.01
        ; (* Approximate ask from close price *)
  ...
  | Ok (Analytics analytics) ->
      console_log
        ( "Received analytics for " ^ analytics.instrument_id ^ " at "
        ^ string_of_float analytics.timestamp ) ;
      (* Analytics messages are informational - log but don't add to market
         data history *)
      ()
  ...
  | Error err -> console_error ("Failed to parse WebSocket message: " ^ err)
...
let send_websocket_message msg =
  ...
  | None -> console_error "WebSocket not connected"

let start_market_data_via_websocket () =
  let control_msg = SimulationControl {action= "start"; parameters= []} in
  send_websocket_message control_msg ;
  ClientState.start_market_data ()

let stop_market_data_via_websocket () =
  let control_msg = SimulationControl {action= "stop"; parameters= []} in
  send_websocket_message control_msg ;
  ClientState.stop_market_data ()

let connect_websocket () =
  let protocol =
    if Js.to_string Dom_html.window##.location##.protocol = "https:" then
      "wss"
    else "ws"
  in
...
let setup_controls () =
  ...
  | Some element ->
      let checkbox =
        (Js.Unsafe.coerce element : Dom_html.inputElement Js.t)
      in
      checkbox##.onchange :=
        Dom.handler (fun _event ->
...
let () =
  Dom_html.window##.onload := Dom.handler (fun _event -> init () ; Js._true)
